...
/**
 * Resolves the location of any sub directories within a test given test on a
 * specific port number. It does so by taking in the port number and then
 * returning a function. The function returned takes in a string of n arguments
 * containing the rest of the path.
 *
 * @example
 * var tempDir = resolveTempDir(1234);
 * // outputs: /home/user/.venus_temp-1234/foo/bar
 * tempDir('foo', 'bar');
 *
 * @param {number} port the port the test is running on
 * @return {function} the function when executes takes in the full path
 */
function resolveTempDir(port) {
  var tempDir = getTempDir({
        dirname: constants.tempDir,
        port: port || constants.port
      }),
      start = [constants.userHome, tempDir];
