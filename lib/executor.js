...
Executor.prototype.init = function(options, onStart) {
  ...
  this.start(this.port, onStart)
    ...
    .then(this.removeExistingTests.bind(this))
    .then(this.handleStaticContent.bind(this, this.config['static']))
    .then(this.addTempTestRoute.bind(this))
    .then(function() {
...
Executor.prototype.removeExistingTests = function(startingPort) {
  var tempRoot = this.resolveVenusTemp('test'),
      def = deferred();
...
/**
 * Resolves location of temporary venus test directory. It takes in n number of
 * parameters and concatentates with the starting directory.
 * @example
 * this.resolveVenusTemp('test', 'foo', 'bar');
 * // outputs: /home/user/.venus_temp-{port}/test/foo/bar
 * @return {string} full path to the temporary test directory
 */
Executor.prototype.resolveVenusTemp = function() {
  var start = [constants.userHome, '.venus_temp-' + this.port],
      end = Array.prototype.slice.call(arguments);
...
Executor.prototype.prepStaticContent = function(paths) {
  var httpRoot = this.resolveVenusTemp('static');
...
Executor.prototype.addTempTestRoute = function() {
  this.app.use('/temp-' + this.port, express.static(this.resolveVenusTemp()));
};
...
Executor.prototype.handleSandboxResource = function(request, response) {
  var testId   = request.params.testid,
      ...
      path     = this.resolveVenusTemp('test', testId, 'resources', resource);
