...
Executor.prototype.init = function(options) {
  ...
  this.runners.done = 0;
  this.initRoutes(config.instance());
...
Executor.prototype.createPhantomRunners = function( options ) {
  ...
  if( !phantomPath ){
    phantomPath = config.instance().get( 'binaries.phantomjs' );
...
// Set up routes    
Executor.prototype.initRoutes = function(config) {
  var app  = this.app,
      port = this.port,
      exec = this,
  ...
  if (routes) {
    ...
    for (routeKey in routes) {
      app.get(
        this.urlNamespace + '/' + routeKey,
        function(request, response) {
          var contents = fs.readFileSync(routes[routeKey]);
