...
Executor.prototype.init = function(options, onStart) {
  ...
  this.start(this.port, onStart)
    ...
    .then(this.removeExistingTests.bind(this, startingPort))
    .then(function() {
...
Executor.prototype.removeExistingTests = function(startingPort) {
  var tempRoot = pathm.resolve(constants.userHome, '.venus_temp', 'test'),
      def = deferred();

      console.log(startingPort, this.port);
  if (startingPort === this.port) {
