...
Executor.prototype.initRoutes = function() {
  ...
  app.post('/test/results/:testid', function(request, response) {
    ...
    if (result.done.failed === 0) {
      ...
      console.log('\n✓'.green + ' ' + result.done.passed.toString().green + content .green +
                  ' ('.green + result.done.runtime.toString().green  + 'ms)'.green);
    }
    else {
      var content = result.done.failed === 1 ? ' test failed' : ' tests failed';
      console.log('\nx'.red + ' ' + result.done.failed.toString().red + ' of '.red + result.done.total.toString().red + content.red + 
                  ' ('.red + result.done.runtime.toString().red  + 'ms)'.red);
    };
    console.log('\r');
