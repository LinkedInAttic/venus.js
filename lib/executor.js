...
Executor.prototype.init = function(config) {
  ...
  if(config.phantom) {
    logger.info( i18n('Creating phantom runners') );
...
Executor.prototype.initRoutes = function() {
  ...
  app.get('/test/:testid', function(request, response) {
    ...
    templateData = {
      frameworkUrls: test.frameworkUrls,
      includeUrls: test.includeUrls,
      testcaseUrl: test.testUrl,
  ...
  app.post('/test/results/:testid', function(request, response) {
    ...
    logger.info( i18n('got results for test %s : %s', testId, request.body) );
  });
}
