...
Executor.prototype.initRoutes = function() {
  ...
  app.get('/test/sandbox/:testid', function(request, response) {
    ...
    fixtureContent = test.annotations['venus-fixture'];
    fixtureId = 'fixture_' + testId;
    dust.loadSource(dust.compile(fixtureContent, fixtureId));
    fixturePartialReference = '{>' + fixtureId + '/}</body>';
    templateFixture = test.fixtureTemplate;
    templateFixture = templateFixture.replace(/<\/body>/,
                                              fixturePartialReference);

    // Set template data, and render the Dust template
    templateFixtureId = 'fixture_template_' + testId;
    dust.loadSource(dust.compile(templateFixture, templateFixtureId));
    templateData = {
...
Executor.prototype.start = function(port) {
  ...
  portscanner.findAPortNotInUse(port, port + 1000, 'localhost', function(err, port) {
    var server = app.listen(port),
        nsocket = io.listen(server);
        nsocket.set('log level', 1);
    logger.info('executor started on ' + hostname + ':' + port);

    /** 
     * Socket IO - Establish Connection and Listen for the "done" event 
     * fired by the client when the tests complete 
     */
    logger.info('socket.io started - executor');
    
    nsocket.sockets.on('connection', function(socket){
      logger.info('Socket.io connection established.');
      socket.on('done', function(result){
        exec.printResults(result);
      });
      socket.on('console.log', function(result){
        logger.info('from console.log:\n'+result);
      });
    });
  });
}
...
/**
 * Print Results from socket.io to the console.
 * @param {Object} result test results
 */
Executor.prototype.printResults = function(result){
  var testId = result.testId,
        test = this.testgroup.tests[testId];
