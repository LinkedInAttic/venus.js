...
Executor.prototype.printResults = function (result) {
  var reporter = this.reporter, test;
  ...
  result.tests.forEach( function(test) {
    ...
    reporter.emit('test-end', test.name);
  });
...
Executor.prototype.start = function(port, onStart) {
  ...
  portscanner.findAPortNotInUse(port, port + 1000, 'localhost', function(err, port) {
    ...
    server.on('listening', function() {
      if (self.testgroup) {
        logger.info('Venus server started at http://' + hostname + ':' + port + ' and is serving ' + self.testgroup.testCount.toString().yellow + ' test suites');
      }
