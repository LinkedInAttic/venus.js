...
Executor.prototype.init = function(options, onStart) {
  ...
  this.start(this.port, onStart).then(function (port) {
    ...
    logger.info('Venus server started at ' + this.protocol + this.hostname + ':' + this.port + ' and is serving ' + testgroup.testCount.toString().yellow + ' test files');
...
Executor.prototype.createTestObjects = function (testPaths) {
  ...
  testFilePaths.forEach(function (testFilePath) {
    ...
    testObjects[testId] = testcase.create({
      path: testFilePath,
      id: testId,
      runUrl: this.protocol + hostname + ':' + this.port + this.urlNamespace + '/' + testId,
  ...
  portscanner.findAPortNotInUse(port, port + 1000, 'localhost', function(err, port) {
    ...
    if (this.useHttps) {
      server = https.createServer({
        key: fs.readFileSync(path.resolve(__dirname, '..', 'ssl', 'privatekey.pem')).toString(),
        cert: fs.readFileSync(path.resolve(__dirname, '..', 'ssl', 'certificate.pem')).toString()
      }, app);
    } else {
