 ...
 * @constructor
 * @param {String} cwd sets the current working directory
 */
 ...
 * @param  {Object}   options settings for configuring this instance.
 * {
 *   'hostname'     : Optional. Sets a hostname.
 *   'coverage'     : Optional. Enables code coverage. Defaults to false.
 *   'noAnnotations': Optional. If true, annotations are not requried for inclusion in code coverage calculations.
 *   'reporter'     : Optional.
 *   'environment'  : Optional.  The environment object to use.
 *   'test'         : list of relative paths specified in the command line arguments
 * }
 * @param  {Function} onStart a callback that is called when the instance is started
 * @return {Promise}          a promise that resolves when the executor has finished starting
 */
...
/**
 * Sets up a reporter
 * @param  {String}   reporter   the name of the reporter module to set
 * @param  {String}   outputFile a filepath where results will be written
 * @return {Reporter}            the newly setup reporter instance
 * @throws {Error}               If unable to instantiate the reporter instance
 */
...
/**
 * Creates UACs (User Account Controls)
 * @param  {TestRun} testgroup a TestRun obect that contains a group of tests in its testArray property
 * @param  {Object}  env       an environment object containing config information
 * @return {Array}             an array of User Account Control objects
 */
...
/**
 * Creates test objects from a comma-separated list of test paths
 * @param  {String} tests a string containing file paths separated by commas
 * @return {Object}       an object containing test objects.  Is an empty object  if tests is not a string.
 */
...
/**
 * Take a list of paths to testcase files and return array of TestCase objects
 * @param  {Array} testPaths     an array of paths to testcase files
 * @return {Array}               an array of resolved test file paths
 * @throws {Error} If a testPath cannot be resolved to a test file
 */
  ...
  testPaths.forEach(function (path) {
    ...
    try {
      ...
      if (stat.isDirectory()) {
        ...
        //If annotations are not required for inclusion or the file at path contains annotations
...
/**
 * Create all test objects.  Binds this.reloadTest function to reload event
 * @param  {Array}  testPaths an array containing paths to the test files
 * @return {Object}           an object containing tests (each accessible as a property with its key equal to the test's ID)
 */
 ...
 * @param  {Object}    config a configuration object.  Has options 'homeFolder' and 'port'
 * @return {Undefined}        undefined
 */
...
/**
 * Initializes routes by binding handlers to each route
 * @return {Undefined} undefined
 */
 ...
 * @return {String}          the friendly, transformed user agent data (family + major)
 */
...
/**
 * Print test results from client including pass/fail status and stack trace (for failed tests)
 * @param  {Object}  result results data indicating the pass/fail outcome in the 'status' property of each test in 'results.tests'
 * @return {Boolean}        Returns false if there are no tests or if this is called before tests are done.  Returns true if no reporter is being used, and does not attempt to print.
 */
...
/**
 * Prints code coverage values
 * @param  {Object}    data test data object containing properties with keys equal to the test file paths
 * @return {Undefined}      undefined
 */
...
/**
 * Shuts down venus, reporting execution time and recording coverage data to file
 * @return {Undefined} Undefined
 */
...
/**
 * Bound to on 'results' event, handles results for each test
 * @param  {Object}             data test result data
 * @param  {Function|Undefined} done calllback called when results have been handled
 * @return {Undefined}          Undefined
 */
 ...
 * @param  {Function} onStart a callback called once server has finished starting
 * @return {Promise}          a Promise that resolves when sever has started and event binding has finished
 */
...
/**
 * Starts a new Executor instance
 * @param  {Object}   config a config object
 * @return {Executor}        an executor instance initialized with the options in config
 */
