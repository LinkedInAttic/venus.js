...
Executor.prototype.removeExistingTests = function(startingPort) {
  var tempRoot = pathm.resolve(constants.userHome, '.venus_temp-' + this.port, 'test'),
      def = deferred();
...
Executor.prototype.initEnvironment = function(config) {
  ...
  // will be used for static content as defined in prepStaticContent
  // since this content is technically "static" it doesn't need to be dynamic
  // or based off of the port which venus is running on. Technically, it could
  // be, but we would have multiple location for which this is being referenced
  // when that feels unnecessary
...
Executor.prototype.addTempTestRoute = function() {
  this.app.use('/temp-' + this.port, express.static(pathm.resolve(constants.userHome, '.venus_temp-' + this.port)));
};
...
Executor.prototype.handleSandboxResource = function(request, response) {
  var testId   = request.params.testid,
      ...
      path     = pathm.resolve(constants.userHome, '.venus_temp-' + this.port, 'test', testId, 'resources', resource);
