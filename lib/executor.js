...
Executor.prototype.init = function(options, onStart) {
  ...
  if (!options.test) {
    options.test = this.getTestFiles(options.rootDirectory);
  }
...
// Get all test files from directory test or tests with sufix .spec.js
Executor.prototype.getTestFiles = function(root_directory) {
  ...
  walk(root_directory, function(err, results) {
    if (err) throw err;
    tests = results.join(",");
  });
