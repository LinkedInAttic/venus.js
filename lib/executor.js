...
Executor.prototype.init = function(options, onStart) {
  ...
  this.reporter = this.setupReporter(options.reporter || this.config.reporter);
  this.reporter.emit('suite');
 ...
 */
Executor.prototype.setupReporter = function (reporter) {
...
Executor.prototype.printResults = function (result) {
  ...
  reporter.emit('file', this.getFriendlyBrowserName(result.userAgent));

  result.tests.forEach(function(test) {
    reporter.emit('start', test.name);

    if (test.status === 'PASSED') {
      reporter.emit('pass', test.message);
    } else {
      reporter.emit('fail', test.message, test.stackTrace);
    }
  });

  reporter.emit('file_end', result.done.runtime);
