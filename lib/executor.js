...
Executor.prototype.createSauceLabsRunners = function( options ){
  var browserName = options.sauceLabsBrowser || 'firefox',
      version = options.sauceLabsVersion || '18',
      platform = options.sauceLabsPlatform || 'Mac 10.6',
      server = options.sauceLabsServer || 'ondemand.saucelabs.com',
      // please input your username and access key for sauce labs here
      username = options.username || 'atran',
      accessKey = options.accessKey || '6b29247c-0b6f-4bb9-89ca-9d903dc7f327',
      myAccount = new sauce({
        username: username,
        password: accessKey
      });
  ...
  logger.verbose( i18n('Name: ' + myAccount.options.username) );
  return this.testgroup.testArray.map(function(test) {
