...
TestCase.prototype.init = function(path, id, runUrl, instrumentCodeCoverage) {
  ...
  this.path                   = testData.path; // Is there any scenario where testData.path != path
...
TestCase.prototype.copyFilesToHttpPath = function (files) {
    ...
    files.forEach(function (path) {
      ...
      if (this.instrumentCodeCoverage && path.instrumentable) {
        ...
        // Symlink the temp location to the actual file...
        mkdirp(symMkdirPath, function () {
          fs.unlink(path.http, function() {
            fs.symlink(path.fs, path.http, 'file');
          });
        });
...
function create(config) {
  ...
  instance.init(config.path, config.id, config.runUrl, config.instrumentCodeCoverage);
  return instance;
}
