...
TestCase.prototype.prepareDependencies = function(settings) {
  ...
  if(lib) {
    ...
    fstools.copy(lib, libDest, copyError);
    this.frameworkUrls.push('/' + libDest.substr( libDest.indexOf('temp/test/'+this.id) ));
  }

  if(adaptor) {
    adaptorDest = pathm.resolve(root + '/' + pathHelper(adaptor).file)
    fstools.copy(adaptor, adaptorDest, copyError);
    this.frameworkUrls.push('/' + adaptorDest.substr( adaptorDest.indexOf('temp/test/'+this.id) ));
  }
  ...
  fstools.copy(this.testPath, testDest, copyError);
