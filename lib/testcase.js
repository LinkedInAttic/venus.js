...
// Initialize
TestCase.prototype.init = function(path, id, runUrl, runPath, instrumentCodeCoverage, hotReload, port) {
  this.path                   = path;
  this.id                     = id;
  ...
  this.port                   = port;
  this.venusTemp              = fsHelper.resolveTempDir(port);
  this.load();
};
...
TestCase.prototype.getHttpRoot = function (testId) {
  ...
  return this.venusTemp('test', testId.toString());
};
...
TestCase.prototype.prepareIncludes = function (annotations) {
  ...
  includes.forEach(function (include) {
    var filePath       = include.path,
        ...
        httpUrl        = '/' + destination.substr(destination.indexOf('run-' + this.port + '/test/' + testId)),
...
TestCase.prototype.prepareResources = function (annotations) {
  ...
  resources.forEach(function (resource) {
    var filePath       = resource.path,
        ...
        httpUrl        = '/' + destination.substr(destination.indexOf('temp-' + this.port + '/test/' + testId)),
