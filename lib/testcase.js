...
TestCase.prototype.init = function(path, id, runUrl) {
  ...
  this.rootPath        = pathm.resolve(this.path, '..');
...
TestCase.prototype.resolveAnnotations = function(annotations) {
  ...
  if (!stringUtil.hasHtml(fixture)) {
    fixture = fs.readFileSync(pathm.join(this.rootPath, fixture)).toString();
    console.log('fixture');
...
TestCase.prototype.loadFixtureTemplate = function(annotations) {
  var fixtureFilePath,
      fixtureName = annotations[annotation.VENUS_TEMPLATE],
      config = this.config,
      src;
